...
const App: React.FC = () => {
  ...
  // Initialize offers with empty array, fetch immediately
  ...
  // Use a ref to prevent race conditions on optimistic updates
  const clickCountRef = useRef(0);

  // 1. Connectivity & Install Prompt Logic (Run once)
  useEffect(() => {
    // Install Prompt
    ...
    else if (isIosDevice) setShowInstallPrompt(true);
  ...
  // 2. Fetch Public Data (Offers) Immediately - Parallel Loading
  useEffect(() => {
  ...
  // 3. Optimized Auth Logic - Non-blocking
  useEffect(() => {
    let mounted = true;
    ...
    const initSession = async () => {
      try {
        ...
        if (mounted) {
          ...
          setLoading(false); // Render UI immediately, don't wait for profile/admin check
  ...
  const handleCompleteOffer = async (offerId: string) => {
    ...
    try {
      // Fire and forget (mostly), but handle error revert
      await trackOfferClick(offerId, user.id);
      
      // Optional: re-verify with server in background
      // const verifiedCount = await fetchUserTotalClicks(user.id);
      // setClickCount(verifiedCount);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        {currentPage === 'admin' && isAdmin ? (
          ...
          <div>
              {/* Stats Card */}
              <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-2xl p-6 mb-8 shadow-xl shadow-indigo-600/10 animate-slide-up text-white transform transition-transform">
